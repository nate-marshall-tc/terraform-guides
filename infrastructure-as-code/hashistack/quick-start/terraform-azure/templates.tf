...
data "template_file" "hashistack_quick_start" {
  ...
  vars = {
    ...
    provider         = "${var.provider}"
    local_ip_url     = "${var.local_ip_url}"
    consul_bootstrap = "${var.hashistack_servers != -1 ? var.hashistack_servers : length(module.network_azure.subnet_public_ids)}"
    ...
    nomad_bootstrap  = "${var.hashistack_servers != -1 ? var.hashistack_servers : length(module.network_azure.subnet_public_ids)}"
